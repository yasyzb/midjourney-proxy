...
public class BeanConfig {
	...
	@Bean
	List<DiscordInstance> discordInstances(List<DiscordAccount> discordAccounts, DiscordHelper discordHelper, RestTemplate restTemplate) {
		...
		for (DiscordAccount account : discordAccounts) {
			DiscordServiceImpl discordService = new DiscordServiceImpl(account.getGuildId(), account.getChannelId(), account.getUserToken(),
	...
	@Bean
	Map<String, WebSocketStarter> webSocketStarterMap(List<DiscordAccount> discordAccounts, ProxyProperties properties, DiscordHelper discordHelper) {
